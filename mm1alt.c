...
float area_num_in_q, area_server_status, mean_interarrival, mean_service,
      ...
      time_next_event[5], total_of_delays, total_delay_sys, total_delay_leave_q;
FILE  *infile, *outfile;
...
void depart(void)  /* Departure event function. */
{
	...
	if (num_in_q == 0) {
        ...
        ++num_custs_delayed;
        delsys = expon(mean_service);
        time_next_event[2] = sim_time + delsys;
		total_delay_sys += delsys;
...
void report(void)  /* Report generator function. */
{
    ...
    fprintf(outfile, "Average delay in system%20.3f minutes\n\n",
            (total_delay_sys + total_of_delays )/ num_custs_delayed);
